...
class Resize(object):
    ...
    def __call__(self, image1, image2, target=None):
        size = self.get_size(image1.size)
        image1 = F.resize(image1, size)
        image2 = F.resize(image2, size)
        if target is None:
            return image1, image2
        target = target.resize(image1.size)
        return image1, image2, target
...
class ColorJitter(object):
    ...
    def __call__(self, image1, image2, target):
        image1 = self.color_jitter(image1)
        #image2 = self.color_jitter(image2)
        return image1, image2, target


class ToTensor(object):
    def __call__(self, image1, image2, target):
        return F.to_tensor(image1), F.to_tensor(image2), target
...
class Normalize(object):
    ...
    def __call__(self, image1, image2, target=None):
        if self.to_bgr255:
            ...
            image1 = image1[[2, 1, 0]] * 255
            image2 = image2[[2, 1, 0]] * 255
